...
socket.on('join_room_response', function(payload){
  ...
  var newHTML = '<p>'+payload.username+' just entered the atmosphere</p>'
  var newNode = $(newHTML);
  newNode.hide();
  ...
  //message handler
  var newHTML = '<p>'+payload.username+' just left the atmosphere</p>';
  var newNode = $(newHTML);
  newNode.hide();
  $('#messages').prepend(newNode);
  newNode.slideDown(1000);
...
socket.on('game_update', function(payload){
  ...
  for(row=0; row < 8; row++){
    ...
    if (old_board[row][column] !== board[row][column]){
      ...
      if (old_board[row][column] === '?' && board[row][column] === ' ') {
        $('#'+row+'_'+column).html('<img src="assets/images/emptytrans.gif" alt="empty square"/>');
      }
    ...
    $('#'+row+'_'+column).removeClass('click');
...
socket.on('game_over', function(payload){
  ...
  $('#game_over').append('<a href="lobby.html?username='+username+'" class="btn btn-success btn-lg btn-active" role="button" aria-pressed=<"true">Well Fought!</a>');
