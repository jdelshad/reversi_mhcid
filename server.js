        ...
        socket.on('play_token',function(payload){
          ...
          // is legit user?
            ...
            if(('undefined' === typeof column) || column < 0 || column > 7){
              var error_message = 'play_tocket can not find payload.column; cmd aborted';
              log(error_message);
              socket.emit('play_token_response' , {
                                                    result: 'fail',
                                                    Message: error_message
            ...
            if(('undefined' === typeof color) || (color != 'white' && color !='black')){
              var error_message = 'play_tocket can not find color; cmd aborted';
              log(error_message);
              socket.emit('play_token_response' , {
                                                    result: 'fail',
                                                    Message: error_message
            ...
            /* if current attempt at play_token is out of turn then error*/
            if(color !== game.whose_turn){
              var error_message = 'play_tocken message played out of turn';
              log(error_message);
              socket.emit('play_token_response',{
                results: 'fail',
...
/* function valid moves */
/* check if rc, contains opposite of who and if addinf dr to r and dc to case
eventually ends in color */
